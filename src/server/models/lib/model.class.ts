...
export class Model<model_type> {
  ...
  bulk_insert = (entries: any[]): any => {
    ...
    const db_query = `INSERT INTO ${this.table} (${props.join(
      ","
  ...
  single_update = (entry: any, _id: number): Promise<any> => {
    ...
    const db_query = `UPDATE ${this.table} SET (${updates.join(
      ","
    )}) RETURNING * WHERE _id = ${_id}`;
    return new Promise((resolve, reject) => {
      this.pool.connect((err, client, done) => {
        if (err) {
          done();
  ...
  destroy_by_id = (_id: number): Promise<any> => {
    return new Promise((resolve, reject) => {
      this.pool.connect((err, client, done) => {
        ...
        client.query(
          `DELETE FROM ${this.table} WHERE _id=$2`,
          [_id],
          (err, result) => {
            done();
            if (err) {
